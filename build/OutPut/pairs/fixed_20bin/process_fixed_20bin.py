...
def main() -> None:
    ...
    output_csv = base_dir / "step_profile_20bin.csv"
    output_png = base_dir / "bragg_let_20bin.png"
    output_txt = base_dir / "sentaurus_input_20bin_for_sentaurus.txt"

    # Read step data with position information for lateral spread calculation
    df = pd.read_csv(input_csv, usecols=[
        "Depth_um", "Edep_keV",
        "PreX_mm", "PreY_mm", "PostX_mm", "PostY_mm"
    ])

    # Convert position to um and calculate average position per step
    ...
    def calc_energy_weighted_spread(group):
        ...
        if E_sum == 0 or len(E) < 2:
            return pd.Series({
                "Edep_sum_keV": E_sum,
                "Count": len(E),
                "LateralSpread_um": 0.1  # default minimum
            })
    ...
    # Group and calculate statistics with energy-weighted spread
    ...
    # Ensure minimum lateral spread for numerical stability
    ...
    # Derivation:
    #   N_L = E_L * 1000 / 3.6 pairs/um (energy to pairs conversion)
    #   N_L,cm = N_L * 1e4 = E_L * 1e7 / 3.6 pairs/cm
    #   A = pi * w_cm^2 = pi * (w * 1e-4)^2 cm^2 (cross-section area)
    #   C_max = N_L,cm / A = (E_L * 1e7 / 3.6) / (pi * w^2 * 1e-8)
    #         = E_L * 1e7 / (3.6 * pi * w^2 * 1e-8)
    #         = E_L / (w^2) * 1e15 / (3.6 * pi)
    #         ≈ 8.84e13 * E_L / w^2
    ...
    print("\nSample output (all units in cm):")
    print(grouped[["z_center_cm", "LET_pairs_cm3", "LateralSpread_cm"]].head())
